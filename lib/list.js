 ...
 * sublist reversing
 * 
 * 1st-index positioning
 * 
 * also can be from node -> to node instead indexes
 */
function sublistReverse(from, to) {
  to = to - from + 1;
  if (to < 1) return;
  let start = this[head];
  let left = null;
  while (from-- > 1 && start !== null) {
    left = start;
    start = start.next;
  }
  ...
  start.next = next;
  if (pointer === null) this[tail] = start;
}
